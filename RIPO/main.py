...
class ObjectDetectionSystem:
    ...
    def load_patterns(self):
        """Ładuje wzorce z folderu wzorców do detekcji."""
        self.patterns = []
        for filename in os.listdir(self.output_folder):
            if filename.endswith(".jpg"):
                img = cv2.imread(os.path.join(self.output_folder, filename), cv2.IMREAD_GRAYSCALE)
                self.patterns.append(img)
                self.logger.info(f"Załadowano wzorzec: {filename}")
    ...
    def process_frame(self, frame):
        ...
        if self.collection_enabled:
            ...
            # Upewniamy się, że wzorce są wczytane i w odcieniach szarości
            if hasattr(self, 'patterns') and self.patterns:
                for pattern in self.patterns:
                    # Konwersja wzorca do odcieni szarości (jeśli nie zostało to już zrobione)
                    if len(pattern.shape) > 2:
                        pattern = cv2.cvtColor(pattern, cv2.COLOR_BGR2GRAY)

                    # Dopasowanie wzorca
                    res = cv2.matchTemplate(gray_frame, pattern, cv2.TM_CCOEFF_NORMED)
                    threshold = 0.7  # próg dopasowania
                    loc = np.where(res >= threshold)

                    # Rysowanie prostokątów wokół wykrytych wzorców
                    for pt in zip(*loc[::-1]):  # zamiana współrzędnych na (x, y)
                        cv2.rectangle(processed_frame, pt, (pt[0] + pattern.shape[1], pt[1] + pattern.shape[0]),
                                      (0, 0, 255), 2)
                        cv2.putText(processed_frame, "Wzorzec", (pt[0], pt[1] - 10),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)

                self.logger.info(f"Wykryto wzorce na obrazie")
            else:
                self.logger.warning("Brak załadowanych wzorców!")
